  ...
  static enable() {
    if (this._instance == undefined || this._instance == null) {
      ...
      process.on('uncaughtException', (err) => {
        ...
        console.log(
          'CrashReporter: No SDK crash detected or crash reporter is disabled!'
        )
        throw err
      })
    }
...
const privateMethods = {
    ...
    FacebookAdsApi.getDefaultApi()
      ...
      .then((data) => {
        if (
          ...
          Http.request('POST', url, params, {}, false, false)
            .then((response) => {
  ...
  parseParam(err: Error) {
    ...
    var params: { [name: string]: string | string[] } = {}
